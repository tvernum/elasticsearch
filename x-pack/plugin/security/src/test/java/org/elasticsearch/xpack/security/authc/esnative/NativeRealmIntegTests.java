...
public class NativeRealmIntegTests extends NativeRealmIntegTestCase {
    ...
    public void testOperationsOnReservedUsers() throws Exception {
        ...
        exception = expectThrows(
            ...
            () -> new ChangePasswordRequestBuilder(client()).username(AnonymousUser.DEFAULT_ANONYMOUS_USERNAME)
                .password("foobar".toCharArray(), hasher)
                .get()
        );
        assertThat(exception.getMessage(), containsString("user [" + AnonymousUser.DEFAULT_ANONYMOUS_USERNAME + "] is anonymous"));
        ...
        exception = expectThrows(IllegalArgumentException.class, () -> preparePutUser(SystemUser.NAME, "foobar", hasher).get());
        assertThat(exception.getMessage(), containsString("user [" + SystemUser.NAME + "] is internal"));

        exception = expectThrows(
            IllegalArgumentException.class,
            () -> new ChangePasswordRequestBuilder(client()).username(SystemUser.NAME).password("foobar".toCharArray(), hasher).get()
        );
        assertThat(exception.getMessage(), containsString("user [" + SystemUser.NAME + "] is internal"));
    ...
    public void testCreateAndChangePassword() throws Exception {
        ...
        ChangePasswordResponse passwordResponse = new ChangePasswordRequestBuilder(
            client().filterWithHeader(Collections.singletonMap("Authorization", token))
        ...
        for (int i = 0; i < negativeLookups; i++) {
            if (anonymousEnabled && roleExists) {
                ClusterHealthResponse response = client().filterWithHeader(
                    Collections.singletonMap("Authorization", basicAuthHeaderValue("joe", new SecureString("s3krit")))
                ).admin().cluster().prepareHealth().get();
