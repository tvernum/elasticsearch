...
public final class TokenService {
    ...
    public void invalidateRefreshToken(String refreshToken, ActionListener<TokensInvalidationResult> listener) {
        ...
        if (Strings.isNullOrEmpty(refreshToken)) {
            ...
            maybeStartTokenRemover();
            final Iterator<TimeValue> backoff = DEFAULT_BACKOFF.iterator();
            findTokenFromRefreshToken(refreshToken, backoff, ActionListener.wrap(searchResponse -> {
                ...
                () -> executeAsyncWithOrigin(
                    ...
                    ActionListener.<BulkResponse>wrap(bulkResponse -> {
                        ...
                        if (retryTokenDocIds.isEmpty() == false && backoff.hasNext()) {
                            ...
                            final Runnable retryWithContextRunnable = client.threadPool()
                                .getThreadContext()
                                .preserveContext(
                                    () -> indexInvalidation(
                                        retryTokenDocIds,
    ...
    public void refreshToken(String refreshToken, ActionListener<Tuple<String, String>> listener) {
        ...
        final Iterator<TimeValue> backoff = DEFAULT_BACKOFF.iterator();
        findTokenFromRefreshToken(refreshToken, backoff, ActionListener.wrap(tokenDocHit -> {
            ...
            if (refreshToken.length() == HASHED_TOKEN_LENGTH) {
                ...
                try {
                    ...
                    if (refreshTokenVersion.before(VERSION_TOKENS_INDEX_INTRODUCED) || unencodedRefreshToken.length() != TOKEN_LENGTH) {
                        logger.debug(
                            "Decoded refresh token [{}] with version [{}] is invalid.",
    ...
    private BytesReference createTokenDocument(
        ...
        assert refreshToken == null || originatingClientAuth != null : "non-null refresh token "
            + refreshToken
            + " requires non-null client authn "
    ...
    private static ElasticsearchSecurityException malformedTokenException() {
        ElasticsearchSecurityException e = new ElasticsearchSecurityException("token malformed", RestStatus.UNAUTHORIZED);
